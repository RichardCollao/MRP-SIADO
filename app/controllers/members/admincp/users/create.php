...
class CreateController extends usersController {
    ...
    private function _send($data) {
        ...
        if (is_empty($errors)) {
            ...
            if (!is_empty($this->_Model->duplicateName($values))) {
            ...
            } elseif ($last_insert_id = $this->_Model->createUser($values)) {
                $MsgBox = new MsgBox();
                ...
                redirect(Path::urlDomain('./permissions/' . $last_insert_id));
            } else {
...
//        redirect(path::urlDomain('./create'));
    ...
    private function _validateForm($data) {
        ...
        if (!array_key_exists($fk_id_establishment, $this->list_establishments)) {
            $errors[] = "El campo <b>Obra</b> es incorrecto.";
        }
