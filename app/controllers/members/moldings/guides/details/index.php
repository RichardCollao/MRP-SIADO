...
class IndexController extends GuidesController {
    ...
    private function _send($data) {
        ...
        if (is_empty($errors)) {
            ...
            if ($this->_Model->addItem($values)) {
                redirect(path::urlDomain('./' . $this->_id_molding_guide));
            }
        }
    ...
    private function _view($data) {
        $data['action_form'] = path::urlDomain('./' . $this->_id_molding_guide);
        ...
        $data['link_finish'] = path::urlDomain('./../' . $this->_molding_guide['fk_id_molding']);
